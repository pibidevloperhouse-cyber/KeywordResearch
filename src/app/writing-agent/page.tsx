        ...
        <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-purple-500/30 font-outfit">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center flex-col items-center gap-6">
                    {!blogContent && (
                        <button
                            ...
                            className="bg-white text-slate-950 hover:bg-purple-500 hover:text-white px-12 py-5 rounded-[1.5rem] font-black transition-all shadow-[0_20px_40px_rgba(255,255,255,0.1)] active:scale-95 disabled:opacity-50 font-outfit tracking-tight flex items-center gap-4 text-xl"
                        >
                            {loading ? <Loader2 className="animate-spin w-8 h-8" /> : "GENERATE BLOG POST"}
            ...
            <AnimatePresence mode="wait">
                {blogContent && (
                    ...
                    >
                        <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden p-8 backdrop-blur-3xl shadow-2xl relative">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 border-b border-white/5 pb-8">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-3 text-purple-400 font-bold tracking-widest text-xs uppercase">
                                        <FileText className="w-4 h-4" />
                                        <span>Generated Content Preview</span>
                                    </div>
                                    <h2 className="text-3xl font-black text-white italic">"{topic || 'Untitled Topic'}"</h2>
                                </div>

                                <div className="w-full md:w-auto">
                                    <button
                                        onClick={nextStep}
                                        className="w-full bg-gradient-to-r from-purple-500 to-purple-600 hover:from-white hover:to-slate-100 text-slate-950 px-8 py-4 rounded-[1.2rem] font-black transition-all shadow-[0_20px_40px_rgba(168,85,247,0.3)] active:scale-95 flex items-center justify-center gap-4 group"
                                    >
